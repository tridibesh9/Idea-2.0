      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border p-6 space-y-4">
            ...
            {channels.map(({ id, label, icon: Icon }) => (
              <button key={id} type="button"
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm transition ${
                  form.channel === id ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 hover:bg-gray-50'
                }`}>
                <Icon size={14} /> {label}
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Customer name"
              value={form.customer_name} onChange={(e) => setForm({ ...form, customer_name: e.target.value })} />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-1">Email</label>
            <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="customer@example.com" type="email"
        ...
        <div>
          ...
          <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Brief subject line"
            value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} />
        </div>

        {/* Complaint Body */}
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1">Complaint *</label>
          <textarea className="w-full border rounded-lg px-3 py-2 text-sm h-40" placeholder="Describe the issue in detail..."
